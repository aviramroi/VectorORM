...
async function main() {
  ...
  const region = process.env.TURBOPUFFER_REGION; // e.g. 'gcp-us-east4', 'aws-us-east-1'
  ...
  try {
    ...
    const allResults = await adapter.search(NAMESPACE, randomVector(), {
      topK: 3,
    });
    for (const r of allResults.records) {
      console.log(
        `  [${(r.score ?? 0).toFixed(4)}] ${r.id} — ${r.metadata["__h_theme"]}`
      );
    }
    ...
    for (const r of engResults.records) {
      console.log(
        `  [${(r.score ?? 0).toFixed(4)}] ${r.id} — ${r.metadata["text"]?.slice(
          0,
          50
        )}...`
      );
    ...
    // ── 5. Fetch by ID ───────────────────────────────────────────────────
    console.log("\n--- Fetch by ID ---");
    const fetched = await adapter.fetch(NAMESPACE, [
      "doc-pricing-1",
      "doc-security-1",
    ]);
    ...
    await adapter.updateMetadata(NAMESPACE, [
      {
        id: "doc-pricing-1",
        metadata: { reviewed: true, reviewer: "cookbook" },
      },
    ]);
    ...
    for await (const batch of adapter.iterate(NAMESPACE, { batchSize: 2 })) {
      console.log(
        `  Batch of ${batch.length}: [${batch.map((r) => r.id).join(", ")}]`
      );
    ...
    console.log(
      `  Vectors: ${stats.vectorCount}, Dimension: ${stats.dimension}`
    );
  } finally {
    // ── Cleanup ────────────────────────────────────────────────────────
